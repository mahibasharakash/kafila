<template>
    ...
    <section class="w-full px-3 sm:px-10">
        ...
        <!-- Title -->
        <div class="text-[25px]"> {{ ___('Testimonials') }} </div>
        ...
        <ul class="w-full flex justify-start items-center">
            ...
            <li class="mb-0">
                <router-link :to="{name:'testimonials'}" class="decoration-0 text-[#21306b] duration-500 hover:text-[#3B497E] p-2 inline-block">
                    {{___(`Blogs`)}}
                </router-link>
            </li>
        </ul>
        ...
        <section class="w-full bg-white p-3 shadow-sm rounded-[0.625rem_0.625rem_0_0] h-[calc(100vh-220px)]">
                        ...
                        <template v-for="(each, index) in paginatedList" :key="index">
                            ...
                            <div class="w-full border border-gray-200 rounded-[0.625rem_0.625rem_0_0] relative overflow-hidden p-5">
                                    ...
                                    <div class="relative">
                                        ...
                                        <div class="absolute mt-2 overflow-hidden drop-shadow-2xl z-10 duration-500 top-auto end-0 w-[150px]" :class="{ 'max-h-[250px]' : isActiveDropdown === index, 'max-h-[0px]' : isActiveDropdown !== index }" @click.stop>
                                            <div class="p-2 bg-white rounded-xl">
                                                <button type="button" class="w-full flex mb-1 justify-start items-center py-3 px-5 cursor-pointer bg-transparent duration-500 hover:bg-gray-200 group/item rounded-xl text-gray-500 hover:text-[#21306b]" @click="dropdownToggle(index);openManageModal(each)">
                                ...
                                <div class="w-full flex justify-center mb-5">
                                    <div v-if="!each.image" class="size-[100px] rounded-full inline-flex justify-center items-center">
                                        100 * 100
                                    </div>
                                    <img :src="each.image" class="size-[100px] rounded-full object-cover bg-cover" alt="image"/>
                                </div>
                                <div class="w-full block text-center font-normal text-xl mb-2">
                                    <div class="truncate-to-2-line">
                                        {{ each.name }}
                                    </div>
                                </div>
                                <div class="w-full block font-normal text-center">
            ...
            <EmptyScreen :title="___('testimonial')" v-if="paginatedList.length === 0 && !listLoading"/>
        ...
        <aside class="bg-white rounded-3xl w-full sm:max-w-[550px] sm:min-w-[550px] p-10 duration-500 origin-top" :class="{ 'translate-y-0 opacity-100' : isActiveManageModal, '-translate-y-1/2 opacity-0' : !isActiveManageModal }" @click.stop>
            <form @submit.prevent="manageApi" class="w-full block">
                ...
                <div class="w-full mb-3">
                    <div class="flex w-full justify-center">
                        <template v-if="!attach_preview">
                            <label for="image"
                                   class="w-full min-w-[200px] max-h-[200px] max-w-[200px] min-h-[200px] rounded-full bg-[#21306b]/25 duration-500 hover:bg-[#21306b]/95 group cursor-pointer flex justify-center items-center">
                                <input id="image" type="file" name="image" hidden="hidden" accept="image/*"
                                       autocomplete="off" @change="attachFile($event)"/>
                                <span>
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                         stroke-width="2"
                                         class="size-[35px] stroke-[#21306b] duration-500 group-hover:stroke-white">
                        ...
                        <template v-if="attach_preview">
                            <div class="relative min-w-[200px] max-h-[200px] rounded-full overflow-hidden">
                                <img :src="attach_preview"
                                     class="w-full max-h-[200px] max-w-[200px] min-h-[200px] object-cover bg-cover"
                                     alt="attach_file"/>
                                <div
                                    ...
                                    <button type="button"
                                            class="size-[45px] inline-flex justify-center items-center p-2 rounded-full bg-rose-400 duration-500 hover:bg-rose-700 cursor-pointer"
                                            @click="removeFile()">
                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                             stroke-width="2" class="size-[25px] stroke-white">
                                            <path stroke-linecap="round" stroke-linejoin="round"
                ...
                <div class="w-full mb-3 block">
                    <label for="name" class="block mb-1 w-full"> {{ ___('Name') }} </label>
                    <input id="name" type="text" name="name" v-model="formData.name"
                           class="w-full block outline-0 rounded-xl bg-white border border-gray-200 duration-500 focus-within:border-[#3B497E] h-[45px] px-5"
                           autocomplete="off"/>
                    <div class="mt-1 text-rose-500 text-[13px]" v-if="error?.name"> {{ error?.name[0] }}</div>
                </div>
                <div class="w-full mb-3 block">
                    <label for="description" class="block mb-1 w-full"> {{ ___('Description') }} </label>
                    <textarea name="description" id="description" v-model="formData.description" cols="30" rows="5"
                    ...
                    <div class="mt-1 text-rose-500 text-[13px]" v-if="error?.description"> {{ error?.description[0] }}
                    </div>
                </div>
    ...
    methods: {
        ...
        async listApi() {
            try {
                ...
                const response = await axios.get(apiRoutes.testimonial, {headers: apiServices.authHeaderContent});
        ...
        async deleteApi() {
            try {
                ...
                await axios.delete(apiRoutes.testimonial + '/' + this.formData.id, {headers: apiServices.authHeaderContent});
                this.closeDeleteModal();
                await this.listApi();
                toast.success(this.lang === "en" ? "Review deleted successfully." : "تم حذف المراجعة بنجاح.")
            } catch (error) {
                console.log(error);
            } finally {
        ...
        async clear() {
            try {
                this.deleteLoading = true;
                await axios.delete(apiRoutes.testimonial + '/clear', {headers: apiServices.authHeaderContent});
                this.closeDeleteModal();
                await this.listApi();
        ...
        async createApi() {
            try {
                ...
                await axios.post(apiRoutes.testimonial, formData, {headers: apiServices.authHeaderMediaContent});
                this.$router.push({name: 'testimonials'});
                toast.success(this.lang === "en" ? "Testimonial created successfully." : "تم إنشاء الشهادة بنجاح.")
            } catch (error) {
        ...
        async updateApi() {
            try {
                ...
                await axios.post(apiRoutes.testimonial + '/' + this.formData.id + '?_method=PUT', formData, {headers: apiServices.authHeaderMediaContent});
                this.$router.push({name: 'testimonials'});
                toast.success(this.lang === "en" ? "Testimonial updated successfully." : "تم تحديث الشهادة بنجاح.")
            } catch (error) {
                this.error = error?.response?.data?.errors
            } finally {
                this.manageLoading = false;
        ...
        async singleApi(data) {
            try {
                this.loading = true;
                const response = await axios.get(apiRoutes.testimonial + `/${data}`, {
