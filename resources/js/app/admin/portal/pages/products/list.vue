<template>
    ...
    <section class="w-full px-3 sm:px-10">
        ...
        <!-- Card body title -->
        <div class="text-[25px]"> {{ ___('Products') }} </div>
        ...
        <section class="w-full bg-white p-3 shadow-sm rounded-[0.625rem_0.625rem_0_0] h-[calc(100vh-220px)]">
            ...
            <section class="w-full block sm:flex items-center justify-between flex-wrap sm:flex-nowrap mb-2.5">
                ...
                <div class="flex justify-end items-center gap-3 w-full">
                    ...
                    <router-link :to="{name:'productManage',params:{id:'new'}}" class="cursor-pointer px-10 h-[45px] text-[14px] font-medium max-w-[120px] min-w-[120px] bg-[#21306b] duration-500 hover:bg-[#3B497E] inline-flex justify-center items-center text-white rounded-[0.625rem]">
                        {{ ___('Add') }}
                    </router-link>
                ...
                <div class="w-full overflow-y-auto pe-2" :class="filteredList.length > 16 ? 'h-[calc(100vh-360px)]' : 'h-[calc(100vh-310px)]'">
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-4 gap-3">
                        ...
                        <template v-for="(each, index) in paginatedList" :key="index">
                            ...
                            <!-- Product data -->
                            <div class="w-full shadow-md border border-gray-100 rounded-xl p-5 relative">
                                <div class="p-7 absolute top-0 end-0 z-3">
                                    <div class="relative">
                                        ...
                                        <div class="absolute mt-2 overflow-hidden drop-shadow-2xl z-10 duration-500 top-auto end-0 w-[150px]" :class="{ 'max-h-[250px]' : isActiveDropdown === index, 'max-h-[0px]' : isActiveDropdown !== index }" @click.stop>
                                            <div class="p-2 bg-white rounded-xl">
                                                <router-link :to="{name:'productManage',params:{id:each.id}}" class="w-full flex mb-1 justify-start items-center py-3 px-5 cursor-pointer bg-transparent duration-500 hover:bg-gray-200 group/item rounded-xl text-gray-500 hover:text-[#21306b]" @click="dropdownToggle(index);">
                                ...
                                <div class="w-full block">
                                    <div class="relative">
                                        ...
                                        <div class="absolute top-0 start-0 p-3 flex justify-start items-center">
                                            <div class="px-3 py-1 rounded-lg m-0 bg-white shadow-md text-center">
                                                <template v-if="each.quantity"> Qty: {{ each.quantity }}</template>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                <div class="p-5 block w-full">
                                    ...
                                    <div class="w-full block font-normal text-gray-500 text-[14px] mb-3">
            ...
            <EmptyScreen :title="___('product')" v-if="paginatedList.length === 0 && !listLoading"/>
...
export default {
    ...
    computed: {
        filteredList() {
            ...
            return this.listData.filter(data => {
                ...
                return title?.toLowerCase().includes(query) || price?.toLowerCase().includes(query);
            })
        },
        totalPages() {
    ...
    watch: {
        ...
        'formData.price'(newVal) {
            const cleaned = newVal.replace(/[^0-9.]/g, '').replace(/(\..*?)\..*/g, '$1');
            if (newVal !== cleaned) {
                this.formData.price = cleaned;
            }
        },
        'formData.quantity'(newVal) {
            const cleaned = newVal.replace(/\D/g, '');
            if (newVal !== cleaned) {
                this.formData.quantity = cleaned;
            }
        }
    },
    data() {
    ...
    methods: {
        ...
        async deleteApi() {
            try {
                ...
                await axios.delete(apiRoutes.product + '/' + this.formData.id, {headers: apiServices.authHeaderContent});
                this.closeDeleteModal();
                await this.listApi();
                toast.success(this.lang === "en" ? "Product deleted successfully." : "تم حذف المنتج بنجاح.")
            } catch (error) {
                console.log(error);
            } finally {
        ...
        async clear() {
            try {
                this.deleteLoading = true;
                await axios.delete(apiRoutes.product + '/clear', {headers: apiServices.authHeaderContent});
                this.closeDeleteModal();
                await this.listApi();
                toast.success(this.lang === "en" ? "Product clear successfully." : "تم مسح المنتج بنجاح.")
            } catch (error) {
                console.log(error);
            } finally {
                this.deleteLoading = false;
