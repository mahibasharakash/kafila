<template>
    ...
    <section class="w-full px-3 sm:px-10">
        ...
        <!-- Card body title -->
        <div class="text-[25px]"> {{ ___('Boxes') }} </div>
        ...
        <ul class="w-full flex justify-start items-center">
            ...
            <li class="mb-0">
                <router-link :to="{name:'boxes'}" class="decoration-0 text-[#21306b] duration-500 hover:text-[#3B497E] p-2 inline-block">
                    {{___(`Boxes`)}}
                </router-link>
            </li>
        </ul>
        ...
        <section class="w-full bg-white p-3 shadow-sm rounded-[0.625rem_0.625rem_0_0] h-[calc(100vh-220px)]">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-4 gap-3">
                        ...
                        <template v-for="(each, index) in paginatedList" :key="index">
                            ...
                            <div class="w-full border border-gray-200 rounded-[0.625rem_0.625rem_0_0] relative overflow-hidden">
                                <div class="py-2 px-3 absolute top-0 end-0">
                                    <div class="relative">
                                        <button type="button" class="size-[35px] inline-flex justify-center items-center bg-transparent duration-500 hover:bg-gray-200 rounded-md cursor-pointer" @click.stop="dropdownToggle(index)">
                                        ...
                                        <div class="absolute overflow-hidden drop-shadow-2xl z-10 duration-500 top-auto end-0 w-[150px]" :class="{ 'max-h-[250px]' : isActiveDropdown === index, 'max-h-[0px]' : isActiveDropdown !== index }" @click.stop>
                                            <div class="p-2 bg-white rounded-xl">
                                                <button type="button" class="w-full flex mb-1 justify-start items-center py-3 px-5 cursor-pointer bg-transparent duration-500 hover:bg-gray-200 group/item rounded-xl text-gray-500 hover:text-[#21306b]" @click="dropdownToggle(index);openManageModal(each.id)">
                                                ...
                                                <button type="button" class="w-full flex justify-start items-center py-3 px-5 cursor-pointer bg-transparent duration-500 hover:bg-gray-200 group/item rounded-xl text-gray-500 hover:text-[#21306b]" @click="openDeleteModal(each.id);dropdownToggle(index);">
                                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" class="size-[16px] duration-500 stroke-gray-500 group-hover/item:stroke-[#3B497E]">
                                ...
                                <div class="p-3 flex justify-center">
                                    <div v-if="!each.image" class="flex justify-center items-center w-full max-w-[250px] min-h-[250px] max-h-[250px] bg-gray-200 rounded-xl">
                                        300 * 200
                                    </div>
                                    <img v-else :src="each.image" class="w-full max-w-[250px] min-h-[250px] max-h-[250px] object-cover bg-cover" alt="cover"/>
                                </div>
                                <div class="pt-2 pb-5 px-3">
                                    <div class="w-full block font-semibold text-[19px] mb-3 text-center">
                                        {{ each.title }}
                                    </div>
                                    <div class="w-full block font-normal text-[14px] text-center">
            ...
            <EmptyScreen :title="___('box')" v-if="paginatedList.length === 0 && !listLoading"/>
        ...
        <aside class="bg-white rounded-3xl w-full sm:max-w-[550px] sm:min-w-[550px] p-10 duration-500 origin-top" :class="{ 'translate-y-0 opacity-100' : isActiveManageModal, '-translate-y-1/2 opacity-0' : !isActiveManageModal }" @click.stop>
            <form @submit.prevent="manageApi" class="w-full block">
                ...
                <div class="w-full mb-3">
                    <div class="w-full flex justify-center">
                        <template v-if="!attach_preview">
                            <label for="upload" class="w-full max-h-[250px] min-h-[250px] rounded-xl bg-[#21306b]/15 duration-500 hover:bg-[#21306b]/35 group cursor-pointer flex justify-center items-center">
                                <input id="upload" type="file" name="upload" hidden="hidden" accept="image/*" autocomplete="off" @change="attachFile($event)"/>
                                <span>
                        ...
                        <template v-if="attach_preview">
                            <div class="relative w-full flex justify-center items-center">
                                <img :src="attach_preview" class="w-full max-w-[250px] max-h-[250px] min-h-[250px] object-cover bg-cover" alt="attach_file"/>
                ...
                <div class="w-full block mb-3">
                    <label for="title" class="w-full block mb-1">
                        {{ ___('Title') }}
                    </label>
                    <input type="text" name="title" id="title" v-model="formData.title" class="w-full block outline-0 rounded-xl bg-white border border-gray-300 duration-500 focus-within:border-[#21306b] h-[45px] px-5" autocomplete="off"/>
                    <div class="mt-1 text-rose-500 text-[13px]" v-if="error?.title"> {{ error?.title[0] }}</div>
                </div>
                <div class="w-full block mb-3">
                    <label for="short_description" class="w-full block mb-1">
                        {{ ___('Short Description') }}
                    </label>
                    <textarea name="short_description" id="short_description" v-model="formData.short_description" cols="30" rows="3" class="w-full block outline-0 rounded-xl bg-white border border-gray-200 duration-500 focus-within:ring-2 focus-within:ring-[#21306b] px-4 text-[13px] h-auto px-5 py-4" autocomplete="off"></textarea>
                    <div class="mt-1 text-rose-500 text-[13px]" v-if="error?.short_description">
                        {{ error?.short_description[0] }}
                    </div>
                </div>
    ...
    methods: {
        ...
        async listApi() {
            try {
                ...
                const response = await axios.get(apiRoutes.box, {headers: apiServices.authHeaderContent});
        ...
        async deleteApi() {
            try {
                ...
                await axios.delete(apiRoutes.box + '/' + this.formData.id, {headers: apiServices.authHeaderContent});
                this.closeDeleteModal();
                await this.listApi();
                toast.success(this.lang === "en" ? "Box deleted successfully." : "تم حذف المربع بنجاح.");
            } catch (error) {
                console.log(error);
            } finally {
        ...
        async clear() {
            try {
                this.deleteLoading = true;
                await axios.delete(apiRoutes.box + '/clear', {headers: apiServices.authHeaderContent});
                this.closeDeleteModal();
                await this.listApi();
                toast.success(this.lang === "en" ? "Box clear successfully." : "تم مسح المربع بنجاح.");
            } catch (error) {
                console.log(error);
            } finally {
                this.deleteLoading = false;
        ...
        async createApi() {
            try {
                ...
                await axios.post(apiRoutes.box, formData, {headers: apiServices.authHeaderMediaContent});
                this.closeManageModal();
                await this.listApi();
                toast.success(this.lang === "en" ? "Box created successfully." : "تم إنشاء الصندوق بنجاح.");
            } catch (error) {
        ...
        async updateApi() {
            try {
                ...
                await axios.post(apiRoutes.box + '/' + this.formData.id + '?_method=PUT', formData, {headers: apiServices.authHeaderMediaContent});
                this.closeManageModal();
                await this.listApi();
                toast.success(this.lang === "en" ? "Box updated successfully." : "تم تحديث الصندوق بنجاح.");
            } catch (error) {
                this.error = error?.response?.data?.errors
            } finally {
                this.manageLoading = false;
        ...
        async singleApi(data) {
            try {
                this.loading = true;
                const response = await axios.get(apiRoutes.box + `/${data}`, {
