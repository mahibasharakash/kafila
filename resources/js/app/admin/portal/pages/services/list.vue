<template>
    ...
    <section class="w-full px-3 sm:px-10">
        ...
        <!-- Card body title -->
        <div class="text-[25px]"> {{ ___('Services') }} </div>
        ...
        <section class="w-full bg-white p-3 shadow-sm rounded-[0.625rem_0.625rem_0_0] h-[calc(100vh-220px)]">
            ...
            <section class="w-full block sm:flex items-center justify-between flex-wrap sm:flex-nowrap mb-2.5">
                ...
                <div class="flex justify-end items-center gap-3 w-full">
                    ...
                    <!-- route -->
                    <router-link :to="{name:'serviceManage',params:{id:'new'}}" class="cursor-pointer px-10 h-[45px] text-[14px] font-medium max-w-[120px] min-w-[120px] bg-[#21306b] duration-500 hover:bg-[#3B497E] inline-flex justify-center items-center text-white rounded-[0.625rem]">
                        {{ ___('Add') }}
                    </router-link>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-4 gap-3">
                        ...
                        <template v-for="(each, index) in paginatedList" :key="index">
                            ...
                            <div class="w-full border border-gray-200 rounded-[0.625rem_0.625rem_0_0] relative overflow-hidden">
                                    ...
                                    <div class="relative">
                                        ...
                                        <div class="absolute mt-2 overflow-hidden drop-shadow-2xl z-10 duration-500 top-auto end-0 w-[150px]" :class="{ 'max-h-[250px]' : isActiveDropdown === index, 'max-h-[0px]' : isActiveDropdown !== index }" @click.stop>
                                            <div class="p-2 bg-white rounded-xl">
                                                <router-link :to="{name:'serviceManage',params:{id:each.id}}" class="w-full flex mb-1 justify-start items-center py-3 px-5 cursor-pointer bg-transparent duration-500 hover:bg-gray-200 group/item rounded-xl text-gray-500 hover:text-[#21306b]" @click="dropdownToggle(index);">
                                ...
                                <div class="py-2 px-3">
                                    <div class="w-full flex flex-wrap font-normal gap-1 mb-3">
                                    ...
                                    <div class="w-full block font-normal text-xl mb-3">
                                        <div class="truncate-to-2-line">
                                            {{ each.title }}
                                        </div>
                                    </div>
                                    <div class="w-full block font-normal mb-3">
                                        <div class="truncate-to-2-line">
            ...
            <EmptyScreen :title="___('service')" v-if="paginatedList.length === 0 && !listLoading"/>
    ...
    methods: {
        ...
        async deleteApi() {
            try {
                ...
                await axios.delete(apiRoutes.service + '/' + this.formData.id, {headers: apiServices.authHeaderContent});
                this.closeDeleteModal();
                await this.listApi();
                toast.success(this.lang === "en" ? "Service deleted successfully." : "تم حذف الخدمة بنجاح.")
            } catch (error) {
                console.log(error);
            } finally {
        ...
        async clear() {
            try {
                this.deleteLoading = true;
                await axios.delete(apiRoutes.service + '/clear', {headers: apiServices.authHeaderContent});
                this.closeDeleteModal();
                await this.listApi();
                toast.success(this.lang === "en" ? "Service clear successfully." : "تم مسح الخدمة بنجاح")
            } catch (error) {
                console.log(error);
            } finally {
                this.deleteLoading = false;
        ...
        async singleApi(data) {
            try {
                this.loading = true;
                const response = await axios.get(apiRoutes.service + `/${data}`, {
