<template>
    ...
    <section class="w-full px-3 sm:px-10">
        ...
        <!-- Card body title -->
        <div class="text-[25px]"> {{ ___('Brands') }} </div>
        ...
        <ul class="w-full flex justify-start items-center">
            ...
            <li class="mb-0">
                <router-link :to="{name:'brands'}" class="decoration-0 text-[#21306b] duration-500 hover:text-[#3B497E] p-2 inline-block">
                    {{___(`Brands`)}}
                </router-link>
            </li>
        </ul>
        ...
        <section class="w-full bg-white p-3 shadow-sm rounded-[0.625rem_0.625rem_0_0] h-[calc(100vh-220px)]">
            ...
            <template v-if="paginatedList.length > 0 && !listLoading">
                ...
                <!-- Brand list -->
                <div class="w-full overflow-y-auto pe-1" :class="filteredList.length > 16 ? 'h-[calc(100vh-360px)]' : 'h-[calc(100vh-310px)]'">
                    <template v-for="(each, index) in paginatedList" :key="index">

                        <!-- Brand data -->
                        <div class="w-full rounded-[0.625rem] border border-gray-200 py-1 px-5 mb-2 flex justify-between items-center">
                            <div class="w-full block font-normal">
                                <div class="block md:hidden"> {{___('Name')}}: </div>
                                {{ each.name }}
                            </div>
                            <div class="relative">
                                ...
                                <div class="absolute mt-1 overflow-hidden drop-shadow-2xl z-10 duration-500 top-auto end-0 w-[150px]" :class="{ 'max-h-[250px]' : isActiveDropdown === index, 'max-h-[0px]' : isActiveDropdown !== index }" @click.stop>
            ...
            <EmptyScreen :title="___('brand')" v-if="paginatedList.length === 0 && !listLoading"/>
        ...
        <aside class="bg-white rounded-3xl w-full sm:max-w-[550px] sm:min-w-[550px] p-10 duration-500 origin-top" :class="{ 'translate-y-0 opacity-100' : isActiveManageModal, '-translate-y-1/2 opacity-0' : !isActiveManageModal }" @click.stop>
            <form @submit.prevent="manageApi" class="w-full block">
                ...
                <div class="w-full block mb-3">
                    ...
                    <input type="text" name="name" id="name" v-model="formData.name" class="w-full block outline-0 rounded-xl bg-white border border-gray-200 duration-500 focus-within:border-[#3B497E] h-[45px] px-5" autocomplete="off"/>
...
export default {
    ...
    methods: {
        ...
        async singleApi(data) {
            try {
                this.loading = true;
                const response = await axios.get(apiRoutes.brand + `/${data}`, {
        ...
        async createApi() {
            try {
                ...
                await axios.post(apiRoutes.brand, this.formData, {headers: apiServices.authHeaderContent});
                this.closeManageModal();
                await this.listApi();
                toast.success(this.lang === "en" ? "Brand created successfully." : "تم إنشاء العلامة التجارية بنجاح.");
            } catch (error) {
                this.error = error?.response?.data?.errors
            } finally {
        ...
        async updateApi() {
            try {
                this.error = {};
                this.manageLoading = true;
                await axios.put(apiRoutes.brand + '/' + this.formData.id, this.formData, {headers: apiServices.authHeaderContent});
                this.closeManageModal();
                await this.listApi();
                toast.success(this.lang === "en" ? "Brand updated successfully." : "تم تحديث العلامة التجارية بنجاح.");
            } catch (error) {
                this.error = error?.response?.data?.errors
            } finally {
                this.manageLoading = false;
        ...
        async deleteApi() {
            try {
                ...
                await axios.delete(apiRoutes.brand + '/' + this.formData.id, {headers: apiServices.authHeaderContent});
                this.closeDeleteModal();
                await this.listApi();
                toast.success(this.lang === "en" ? "Brand deleted successfully." : "تم حذف العلامة التجارية بنجاح.");
            } catch (error) {
                console.log(error);
            } finally {
        ...
        async clear() {
            try {
                this.deleteLoading = true;
                await axios.delete(apiRoutes.brand + '/clear', {headers: apiServices.authHeaderContent});
                this.closeDeleteModal();
                await this.listApi();
                toast.success(this.lang === "en" ? "Brand clear successfully." : "تم مسح العلامة التجارية بنجاح.");
            } catch (error) {
                console.log(error);
            } finally {
                this.deleteLoading = false;
