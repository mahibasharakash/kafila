<template>
    ...
    <!-- breadcrumb -->
    <Breadcrumb :title="___(`Service Details`)"
                :links="[ { label: ___(`Home`), href: 'home' }, { label: ___(`Services`), href: 'services' }, { label: ___(`Service Details`), href: 'serviceDetails' } ]"></Breadcrumb>
    ...
    <section class="w-full py-3 sm:py-16 block">
        <div class="mx-auto max-w-[1200px] w-full px-5 sm:px-10">
            ...
            <template v-if="singleData?.description">
                ...
                <img :src="singleData?.image" class="object-cover bg-cover w-full h-[350px] my-10 rounded-2xl"
                ...
                <div class="mt-5 flex justify-start items-center gap-2 flex-wrap">
                    <template v-for="subEach in singleData?.tags.split(', ')">
                        <div
                            class="outline-0 border-0 bg-gray-200 duration-500 hover:bg-gray-300 px-3 py-1 rounded-lg text-[14px]">
                            {{ subEach }}
                        </div>
                    </template>
                </div>
                    ...
                    <form @submit.prevent="feedback()">
                        ...
                        <div class="block text-[14px] px-3">
                            <button type="submit" v-if="!messageLoading"
                                    class="min-w-[170px] max-w-[170px] h-[50px] btn-theme">
                                {{ ___(`Send Message`) }}
                            </button>
    ...
    methods: {
        async feedback() {
            try {
                ...
                this.reviewParam = {type: 'service', name: '', message: ''};
            } catch (error) {
        ...
        async singleApi(data) {
            try {
                this.loading = true;
                const response = await axios.get(apiRoutes.service + `/${data}`, {headers: apiServices.authHeaderContent});
        ...
        async fetchReviewData() {
            try {
                ...
                const response = await axios.get(apiRoutes.review + `?type=service&id=${this.singleData.id}`, {headers: apiServices.authHeaderContent});
